...
| Problem | Cause | Fix |
|---------|-------|-----|
| White screen | JS bundle not loading | Hard refresh (Ctrl+Shift+R), check frontend logs |
| Only 10 careers | API not reachable, frontend uses fallback data | Check if backend is running, check Traefik routing |
| Support page error | Career offers not seeded | Check backend logs for seed_career_offers output |
| 404 on all pages | Traefik routing broken | Check for duplicate containers on internal-web network |
| 502 Bad Gateway | Containers still starting | Wait 2 minutes, check logs |
| API returns HTML | Traefik sending /api to frontend instead of backend | Verify docker-compose labels, check for conflicting containers |
| CORS errors | Domain not in ALLOWED_HOSTS | Add domain to `backend/config/settings.py` CORS_ALLOWED_ORIGINS |

To check for duplicate containers (a common issue):

```bash
sudo docker network inspect internal-web --format '{{range .Containers}}{{.Name}} {{end}}'
```

If you see old containers with similar names, stop and remove them:
...
|----------|-------|---------|
| REACT_APP_API_URL | https://fec-roadmap.precis.tu-darmstadt.de/api | Backend API base URL, baked into JS at build time |

### Backend (in config/settings.py)

| Setting | Value |
|---------|-------|
| ALLOWED_HOSTS | fec-roadmap.precis.tu-darmstadt.de, localhost, 127.0.0.1 |
| CORS_ALLOWED_ORIGINS | http://localhost:3000, https://fec-roadmap.precis.tu-darmstadt.de |
